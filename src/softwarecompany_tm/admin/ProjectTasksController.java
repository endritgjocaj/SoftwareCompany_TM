 ...
 * @author My-Pc
 */
public class ProjectTasksController implements Initializable {
    ...
    public void getProject(Projects project){
        ...
        projectTasks.setText("Project: " + project.getProjectName());
        
        projectChoiceBox.getItems().add(project);
        projectChoiceBox.setValue(project);
        ...
        for(Employees employee : employeesList){
            if(employee.getSector().equals(project.getSector())){
                ...
                for(Technologies technology: employee.getTechnologiesList()){
                    if(project.getTechnologiesList().contains(technology)){
        ...
        setEmployeesToTask(empList);
        
        if(get(actualDate) > get(project.getDueDate())){
    ...
    void setEmployeesToTask(List<Employees> empList){
        ...
        for(Employees employee : empList){
            if(!frontEndComboBox.getItems().contains(employee) && !backEndComboBox.getItems().contains(employee)){
    ...
    public void loadData(){
        ...
        for(Tasks task : list){
            if(task.getProject().getId().equals(selectedProject.getId())){
                tasksList.add(task);
            }
        }
    ...
    private void addTask(ActionEvent event) {
        ...
        if(projectChoiceBox.getValue() == null){
            errorMessage += "You should select one project!\n\n";
        }
        ...
        for(Projects prj : projectsList){
            if(prj.equals(projectChoiceBox.getValue())){
                projectStartDate = prj.getStartDate();
                projectDueDate = prj.getDueDate();
            }
        }
    ...
    private void getBackImage(MouseEvent event) {
        Parent parent = null;
        FXMLLoader loader = loader = new FXMLLoader(getClass().getResource("Projects.fxml"));
